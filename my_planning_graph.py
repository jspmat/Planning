...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        pos_symbols = set()
        neg_sybols = set()
        #check the current state of literals
        for s in self.s_levels[level]:
            if s.is_pos:
                pos_symbols.add(s.symbol)
            else:
                neg_sybols.add(s.symbol)
        #for each possible action check if all the preconditions are filled    
        for action in self.all_actions:
            if set(pos_symbols).issuperset(set(action.precond_pos)) and set(neg_sybols).issuperset(set(action.precond_neg)):
                action_node = PgNode_a(action)
                for s in self.s_levels[level]:
                    if (s.symbol in action.precond_pos and s.is_pos == True) or (s.symbol in action.precond_neg and s.is_pos == False):
    ...
    def add_literal_level(self, level):
        ...
        #loop through all actions and add all possible effects to the next S level
        for action in self.a_levels[level-1]:
            for node in action.effnodes:
                self.s_levels[level].add(node)
        #for each literal in the new S level, connect it with correct A nodes
        for literal in self.s_levels[level]:
            for action in self.a_levels[level-1]:
                if literal in action.effnodes:
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        return (len(set(node_a1.action.effect_add).intersection(node_a2.action.effect_rem)) +
                len(set(node_a2.action.effect_add).intersection(node_a1.action.effect_rem))) != 0
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        return (len(set(node_a1.action.effect_rem).intersection(node_a2.action.precond_pos)) +
                len(set(node_a1.action.effect_add).intersection(node_a2.action.precond_neg)) +
                len(set(node_a2.action.effect_add).intersection(node_a1.action.precond_neg)) +
                len(set(node_a2.action.effect_rem).intersection(node_a1.action.precond_pos))) != 0
        ...
        for goal in self.problem.goal:
            ...
            for level in range(len(self.s_levels)):
                for snode in self.s_levels[level]:
                    if snode.symbol == goal and snode.is_pos:
                        ok = True
